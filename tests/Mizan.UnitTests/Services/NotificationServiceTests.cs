...
public class NotificationServiceTests
{
    ...
    public async Task MarkAsReadAsync_ForAnotherUsersNotification_ThrowsNotFoundException()
    {
        ...
        var notification = Notification.CreateInstallmentReminder(1, 10, 20, "أحمد", 500m, DateTime.UtcNow.Date, 1);
        db.Set<Notification>().Add(notification);
        await db.SaveChangesAsync();

        // User 2 attempts to mark user 1's notification as read
        await Assert.ThrowsAsync<NotFoundException>(
            () => service.MarkAsReadAsync(ownerUserId: 2, notificationId: notification.Id));
    }
    ...
    public async Task MarkAsReadAsync_ForOwnNotification_MarksAsReadSuccessfully()
    {
        ...
        await service.MarkAsReadAsync(ownerUserId: 1, notificationId: notification.Id);
    ...
    public async Task GetPagedAsync_ClampsPageAndPageSize()
    {
        ...
        for (int i = 0; i < 60; i++)
        {
            db.Set<Notification>().Add(
                Notification.CreateInstallmentReminder(1, 1, i + 1, $"طرف {i}", 100m, DateTime.UtcNow.Date, 1));
        }
        await db.SaveChangesAsync();

        // Request page = -5, pageSize = 200 -> should clamp to page 1, pageSize 50
        var result = await service.GetPagedAsync(ownerUserId: 1, page: -5, pageSize: 200, unreadOnly: false);
    ...
    public async Task GetPagedAsync_WhenUnreadOnlyIsTrue_ReturnsOnlyUnreadNotifications()
    {
        ...
        var n1 = Notification.CreateInstallmentReminder(1, 1, 1, "طرف 1", 100m, DateTime.UtcNow.Date, 1);
        var n2 = Notification.CreateInstallmentReminder(1, 1, 2, "طرف 2", 200m, DateTime.UtcNow.Date, 2);
        n2.MarkAsRead();

        db.Set<Notification>().AddRange(n1, n2);
        await db.SaveChangesAsync();

        var result = await service.GetPagedAsync(ownerUserId: 1, page: 1, pageSize: 20, unreadOnly: true);
    ...
    public async Task GetUnreadCountAsync_ReturnsCorrectCount()
    {
        ...
        var n1 = Notification.CreateInstallmentReminder(1, 1, 1, "طرف 1", 100m, DateTime.UtcNow.Date, 1);
        var n2 = Notification.CreateInstallmentReminder(1, 1, 2, "طرف 2", 200m, DateTime.UtcNow.Date, 2);
        var n3 = Notification.CreateInstallmentReminder(1, 1, 3, "طرف 3", 300m, DateTime.UtcNow.Date, 0);
        n3.MarkAsRead();

        db.Set<Notification>().AddRange(n1, n2, n3);
        await db.SaveChangesAsync();

        var response = await service.GetUnreadCountAsync(ownerUserId: 1);
    ...
    public async Task MarkAllAsReadAsync_MarksAllNotificationsAsReadForUser()
    {
        ...
        var n1 = Notification.CreateInstallmentReminder(1, 1, 1, "طرف 1", 100m, DateTime.UtcNow.Date, 1);
        var n2 = Notification.CreateInstallmentReminder(1, 1, 2, "طرف 2", 200m, DateTime.UtcNow.Date, 2);
        var n3OtherUser = Notification.CreateInstallmentReminder(2, 2, 3, "طرف آخر", 300m, DateTime.UtcNow.Date, 0);

        db.Set<Notification>().AddRange(n1, n2, n3OtherUser);
        await db.SaveChangesAsync();

        await service.MarkAllAsReadAsync(ownerUserId: 1);

        var user1Notifications = await db.Set<Notification>().Where(n => n.OwnerUserId == 1).ToListAsync();
