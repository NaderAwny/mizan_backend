...
- **الإشعار (Notification)**: `id` (int), `owner_user_id` (FK → User), `type` (InstallmentReminder = 0), `title` (string, max 150), `message` (string, max 500), `transaction_id` (int?, nullable FK), `installment_id` (int?, nullable FK), `is_read` (bool, default false), `created_at`.
...
## 7. خدمة التذكيرات التلقائية في الخلفية (Background Reminders Job)
